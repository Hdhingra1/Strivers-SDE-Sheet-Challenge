...
{
    if (!root)
        return ""; // if root is null return empty string
    ...
    while (!q.empty())
    {
        ...
        q.pop();
        if (!curr)
            s.append("$,"); // if curr is null append $ to string
        else
            s.append(to_string(curr->data) + ","); // else append curr->data to string
...
TreeNode<int> *deserializeTree(string &serialized)
{
    ...
    while (!q.empty())
    {
        ...
        if (str == "$")
        { // if str is $ then curr->left is null
            curr->left = NULL;
        }
        else
        {
        ...
        getline(s, str, ','); // read next value from string stream
        if (str == "$")
        { // if str is $ then curr->right is null
            curr->right = NULL;
        }
        else
        {
